            ...
            {previews.map(({ value, url, error }) =>
              viewMode === "grid" ? (
                ...
                <div key={value}
                  ...
                  <span className="flex-1 text-sm font-bold text-slate-700 truncate">{value}</span>
                  {url && (
                    <a href={url} download={`${value}.png`}
...
export default function App() {
  ...
  useEffect(() => {
    const dotsOptions = {
      ...
      ...(useGradient
        ? {
          gradient: {
            type: "linear", rotation: (gradAngle * Math.PI) / 180,
  ...
  const inputCls = "w-[90%] px-3 py-2.5 rounded-xl border-2 border-slate-200 bg-slate-50 text-slate-800 font-medium text-sm placeholder:text-slate-300 focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition";
      ...
      <div className="flex flex-col lg:flex-row gap-8 w-full max-w-6xl items-start">
        ...
        <div className="flex-1 rounded-3xl bg-white shadow-2xl overflow-hidden border border-slate-100">
          ...
          <div className="px-6 py-5 bg-gradient-to-r  ">
            <div>
              <a href="https://printe.in" target="_blank" rel="noopener noreferrer">
                    ...
                    <div>
                      ...
                      <div className="flex gap-3 mt-2 items-center">
                        <label className="flex items-center gap-2 cursor-pointer">
                          ...
                          <span className="text-xs text-slate-500">{useGradient ? "From" : "Color"}</span>
                        </label>
                        {useGradient && (
                          <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <div>
                      ...
                      <label className="flex items-center gap-3 cursor-pointer">
                        <input type="color" value={textColor} onChange={e => setTextColor(e.target.value)}
                          className="w-10 h-10 rounded-xl border-2 border-slate-200 cursor-pointer p-0.5 shadow-sm" />
                        <span className="text-sm text-slate-500">{textColor}</span>
                      </label>
                    </div>
                  </div>
                )}
